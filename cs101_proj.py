...
def create_data_structure(s):
	...
	while i < l:
		p1 = s.find(' ')
		user = s[:p1]
		p2 = s.find('connected to')
		p3 = s.find('.')
		connection = s[p2 + 13: p3].split(', ')
		p4 = s.find('play')
		p5 = s.find('.', p4)
		game = s[p4 + 5: p5].split(', ')
		network[user] = [connection, game]
		s = s[p5 + 1: ]
		i = i + 2
	return network
...
def find_path_to_friend(network, user_A, user_B, visited = None):
	if  user_A in network and user_B in network:
		...
		if target in network[source][0]:
			path.append(target)
			return path
			
		else:
			for node in network[source][0]:
				if node not in visited:
					return path + find_path_to_friend(network, node, user_B,visited)
